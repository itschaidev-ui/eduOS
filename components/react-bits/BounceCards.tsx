...
export default function BounceCards({
  ...
  const pushSiblings = (hoveredIdx: number) => {
    ...
    const itemCount = images.length || Children.count(children);
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      if (i === hoveredIdx) {
        ...
        gsap.to(target, {
          ...
          delay,
          overwrite: 'auto',
          zIndex: i // Reset z-index for siblings
        });
      }
    }
  };
  ...
  const renderContent = () => {
      if (images.length > 0) {
          return images.map((src, idx) => (
            <div
                ...
                style={{
                transform: transformStyles[idx % transformStyles.length] ?? 'none'
                }}
      ...
      return Children.map(children, (child, idx) => (
        <div
            key={idx}
            className={`card card-${idx} custom-content`}
            style={{
            transform: transformStyles[idx % transformStyles.length] ?? 'none'
            }}
            onMouseEnter={() => pushSiblings(idx)}
